...
ESErrorCode process_start(const string& name, const string& command, const string& currentDirectory, const vector<string>& arguments, process_t* p) {
	...
	sec.lpSecurityDescriptor = NULL;
	sec.nLength = sizeof(SECURITY_ATTRIBUTES);
	if (!CreateProcess(NULL, (char*)c.c_str(), &sec, &sec, TRUE, 0, NULL, currentDirectory.c_str(), &p->startupInfo, &p->processInfo))
...
ESErrorCode process_share_socket(process_t* p, SOCKET hostSocket, mutex_t lock) {
	...
	BOOL result = DuplicateHandle(GetCurrentProcess(), lock, p->processInfo.hProcess, &duplicatedHandle, 0, TRUE, DUPLICATE_SAME_ACCESS);
	if (!result) {
		closesocket(hostSocket);
		return ESERR_PIPE_WRITE;
	}
